...
class ChromosomeSNP():
    ...
    def draw_chromosomes_and_snps(self, chromosome_lengths, file_path):
        ...
        y_positions = np.linspace(10, 10 * n, n)
        ...
        for i, length in enumerate(chromosome_lengths):
            ...
            y = y_positions[i]
            ax.plot([x_start, x_end], [y, y], color='gray', alpha=0.5, linewidth=8)
            ...
            ax.scatter(snp_positions, [y] * len(snp_positions), color='red', s=10, label='SNPs' if i == 0 else "")
...
file_path = 'C:\genome\LR-66_marker_design\primers_LR-66-590_show.csv'  # Replace this with your actual file path
